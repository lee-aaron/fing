...
pub async fn match_storage_cmds(client: &dyn Storage, subcmd: Option<(&str, &ArgMatches)>) -> bool {
    match subcmd {
        Some(("get", v)) => print_result(client.get(v.get_one::<String>("key").unwrap()).await),
        Some(("set", v)) => {
            let kv = get_kv(v);
        ...
        Some(("list-get", v)) => print_result(client.list_get(v.get_one::<String>("key").unwrap()).await),
        Some(("list-append", v)) => {
            let kv = get_kv(v);
        ...
        Some(("clock", v)) => match v.get_one::<String>("clock").unwrap().parse::<u64>() {
...
fn get_kv(matches: &ArgMatches) -> KeyValue {
    KeyValue {
        key: matches.get_one::<String>("key").unwrap().to_string(),
        value: matches.get_one::<String>("value").unwrap().to_string(),
    }
}
